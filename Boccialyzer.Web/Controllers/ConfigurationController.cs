...
namespace Boccialyzer.Web.Controllers
{
    ...
    public class ConfigurationController : Controller
    {
        ...
        #endregion
        #region # ConfigurationController Конструктор

        /// <summary>
        /// ConfigurationController Конструктор
        /// </summary>
        /// <param name="configurationRepository">Репозиторій роботи з налаштуваннями</param>
        ...
        {
            var result = await _configurationRepository.GetPaged(pageNumber, pageSize, filter, order);
        ...
        public async Task<IActionResult> Get(string id)
        {
            ...
            var result = await _configurationRepository.GetByIdAsync(itemId);
            if (result.Result == OperationResult.Ok) return StatusCode(200, result.Value);
            return StatusCode(422, result.Message);
        }
        ...
        [ProducesResponseType(422)]
        [HttpPost]
        public async Task<IActionResult> Post([FromBody] Configuration item)
        {
            if (item == null) return StatusCode(422, "Відсутні данні.");

            var result = await _configurationRepository.CreateAsync(item);
        ...
        [ProducesResponseType(422)]
        [HttpPut]
        public async Task<IActionResult> Put([FromBody]Configuration item)
        {
            if (item == null) return StatusCode(422, "Відсутні данні.");
            var result = await _configurationRepository.UpdateAsync(item);
        ...
        public async Task<IActionResult> Delete(string id)
        {
            ...
            var result = await _configurationRepository.DeleteAsync(itemId);
