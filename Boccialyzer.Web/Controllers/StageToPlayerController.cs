...
namespace Boccialyzer.Web.Controllers
{
    ...
    [Route("api/StageToPlayer")]
    [Authorize]
    [ApiController]
    public class StageToPlayerController : Controller
    {
        #region # Local variables

        private readonly IStageToPlayerRepository _stageToPlayerRepository;
        ...
        /// Constructor
        /// </summary>
        /// <param name="stageToPlayerRepository"></param>
        public StageToPlayerController(IStageToPlayerRepository stageToPlayerRepository)
        {
            _stageToPlayerRepository = stageToPlayerRepository;
        }
        ...
        {
            var result = await _stageToPlayerRepository.GetPaged(pageNumber, pageSize, filter, order);
        ...
        public async Task<IActionResult> Get(string id)
        {
            ...
            var result = await _stageToPlayerRepository.GetByIdAsync(itemId);
            if (result.Result == OperationResult.Ok) return StatusCode(200, result.Value);
            return StatusCode(422, result.Message);
        }
        ...
        [ProducesResponseType(422)]
        [HttpPost]
        public async Task<IActionResult> Post([FromBody] StageToPlayer item)
        {
            if (item == null) return StatusCode(422, "Відсутні данні.");
            //if (string.IsNullOrEmpty(item.Name)) return StatusCode(422, "Відсутня назва.");

            var result = await _stageToPlayerRepository.CreateAsync(item);
        ...
        [ProducesResponseType(422)]
        [HttpPut]
        public async Task<IActionResult> Put([FromBody] StageToPlayer item)
        {
            if (item == null) return StatusCode(422, "Відсутні данні.");
            var result = await _stageToPlayerRepository.UpdateAsync(item);
        ...
        public async Task<IActionResult> Delete(string id)
        {
            ...
            var result = await _stageToPlayerRepository.DeleteAsync(itemId);
