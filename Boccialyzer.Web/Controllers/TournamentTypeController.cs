...
namespace Boccialyzer.Web.Controllers
{
    ...
    public class TournamentTypeController : Controller
    {
        ...
        /// <summary>
        /// TournamentTypeController constructor
        /// </summary>
        /// <param name="tournamentTypeRepository"></param>
        public TournamentTypeController(ITournamentTypeRepository tournamentTypeRepository)
        {
        ...
        {
            var result = await _tournamentTypeRepository.GetPaged(pageNumber, pageSize, filter, order);
        ...
        public async Task<IActionResult> Get(string id)
        {
            ...
            var result = await _tournamentTypeRepository.GetByIdAsync(itemId);
            if (result.Result == OperationResult.Ok) return StatusCode(200, result.Value);
            return StatusCode(422, result.Message);
        }
        ...
        [ProducesResponseType(422)]
        [HttpPost]
        public async Task<IActionResult> Post([FromBody] TournamentType item)
        {
            if (item == null) return StatusCode(422, "Відсутні данні.");
            if (string.IsNullOrEmpty(item.Name)) return StatusCode(422, "Відсутня назва.");

            var result = await _tournamentTypeRepository.CreateAsync(item);
        ...
        [ProducesResponseType(422)]
        [HttpPut]
        public async Task<IActionResult> Put([FromBody] TournamentType item)
        {
            if (item == null) return StatusCode(422, "Відсутні данні.");
            var result = await _tournamentTypeRepository.UpdateAsync(item);
        ...
        public async Task<IActionResult> Delete(string id)
        {
            ...
            var result = await _tournamentTypeRepository.DeleteAsync(itemId);
